...
Diferenciando por si fue una transacción fraudulenta o no. ¿Hay diferencias entre ambas transacciones? ¿Qué información brindan los boxplots?
...
ggplot(data, aes(x = age, fill = fraud)) +
  geom_bar(binwidth = 5, position = "identity", alpha = 0.5) +
  labs(title = "Histogramas de Edad diferenciados por Sexo",
       x = "Edad",
       y = "Frecuencia") +
  theme_minimal()
```
